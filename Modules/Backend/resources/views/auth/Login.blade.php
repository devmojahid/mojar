...
@section('content')
    <h2 class="h3 text-center mb-3">
        ...
        {{ trans('backend::auth.login_to_your_account', ['app' => get_config('title', 'Mojar CMS')]) }}
    </h2>
    <form class="form-ajax-handle" action="{{ route('admin.login') }}" method="POST">
        @csrf
    ...
    @if (get_config('user_registration', true))
        <div class="text-center text-muted mt-3">
            {{ trans('backend::auth.dont_have_an_account') }} <a href="{{ route('admin.register') }}"
                tabindex="-1">{{ trans('backend::auth.sign_up') }}</a>
        </div>
    @endif
@endsection
